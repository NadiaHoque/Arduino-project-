...
#define DHTPIN D1 // what digital pin we're connected to NodeMCU (D6)
...
#define AIO_USERNAME    "shemonto21"
#define AIO_KEY         "89cb250df8a04d93b6d0b13c54e7f7f9"
...
Adafruit_MQTT_Publish hum2= Adafruit_MQTT_Publish(&mqtt, AIO_USERNAME "/feeds/hum2");
...
void loop() {
  ...
  MQTT_connect();
      ...
      ctg = t;
      Serial.print("*********");
      Serial.println( String(ctg).c_str());
  ...
  Serial.print("...");
  if (! hum2.publish(String(h).c_str())) {
...
void MQTT_connect() {
  int8_t ret;
  delay(1200);  // Stop if already connected.
  if (mqtt.connected()) {
    return;
  }
